...
from CustomModules import bot_directory, log_handler
    ...
    async def on_app_command_error(
        ...
        if isinstance(error, discord.app_commands.MissingPermissions):
            ...
            try:
                try:
                    await interaction.response.send_message(
                        f"Error! Try again.", ephemeral=True
                    )
                except:
                    try:
...
class Functions:
    ...
    async def auto_publish(
        ...
        try:
            ...
            if e.code == 50068:
                discord_logger.info(
                    f"Message {message.id} in channel {channel.id} on guild {message.guild.id} is not an announcement message. (Type {message.type.value})"
                )
            elif e.code == 40033:
                discord_logger.info(
                    f"Message {message.id} in channel {channel.id} on guild {message.guild.id} is already published."
                )
            elif e.status == 503 and e.code == 0:
                if retries > 0:
                    discord_logger.info(
                        f"Discord is currently unavailable. Retrying to publish message {message.id} in channel {channel.id} on guild {message.guild.id}. Retries left: {retries}"
                    )
                    await asyncio.sleep(5)
                    await Functions.auto_publish(
                        message, channel, permissions, retries=retries - 1
                    )
                    return
                else:
                    discord_logger.warning(
                        f"Failed to publish message {message.id} in channel {channel.id} on guild {message.guild.id} after retries."
                    )
            else:
                raise
        except discord.NotFound:
            discord_logger.info(
                f"Message {message.id} in channel {channel.id} on guild {message.guild.id} not found."
            )
        except Exception as e:
            if not message.flags.crossposted:
                discord_logger.error(
                    f"Error publishing message in {channel.id} on {message.guild.id}: {e}"
                )
        ...
        for channel in channels:
            try:
                ...
                invite: discord.Invite = await channel.create_invite(
                    reason=reason, max_age=60, max_uses=1, unique=True
                )
        ...
        for guild in bot.guilds:
            ...
            try:
                await guild_owner.send(
                    f"Broadcast from the owner of the bot:\n{message}"
                )
                success += 1
...
async def permissions_command(
    ...
    if (
        ...
        if choice == "explain":
            await interaction.followup.send(
                "In order for this bot to be able to publish messages, he needs the following permissions for each channel he publishes messages for:\n`View Channel`, `Send Messages`, `Manage Messages` and `Read Message History`.",
                ephemeral=True,
            )
        elif choice == "check":
            if channel is None:
            ...
            if isinstance(channel, discord.TextChannel):
                if not channel.is_news():
                    await interaction.followup.send(
                        "The specified channel is not an announcement channel.",
                        ephemeral=True,
                    )
                    return
                ...
                missing_permissions = [
                    perm for perm in needed_permissions if not getattr(perms, perm)
                ]

                if interaction.guild.me.guild_permissions.administrator:
                    await interaction.followup.send(
                        "The bot has Administrator, so he has all the necessary permissions in this channel.",
                        ephemeral=True,
                    )
                elif not missing_permissions:
                    await interaction.followup.send(
                        "The bot has all the necessary permissions in this channel.",
                        ephemeral=True,
                    )
                else:
                    await interaction.followup.send(
                        f'The bot is missing the following permissions in this channel: {", ".join(missing_permissions)}.\nYou can also give him Administrator.',
                        ephemeral=True,
                    )
            else:
                await interaction.followup.send(
        ...
        await interaction.followup.send(
            "You need the `manage_roles` or `manage_channels` permission to use this command.",
            ephemeral=True,
        )
...
async def botinfo_command(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="Created at",
        value=bot.user.created_at.strftime("%d.%m.%Y, %H:%M:%S"),
        inline=True,
    )
    ...
    embed.add_field(
        name="Shard ID",
        value=f"{interaction.guild.shard_id if interaction.guild else 'N/A'}",
        inline=True,
    )
    embed.add_field(name="\u200b", value="\u200b", inline=True)
    ...
    embed.add_field(
        name="Repo",
        value=f"[GitHub](https://github.com/Serpensin/DiscordBots-AutoPublisher)",
        inline=True,
    )
    embed.add_field(
        name="Invite",
        value=f"[Invite me](https://discord.com/oauth2/authorize?client_id={bot.user.id})",
        inline=True,
    )
    embed.add_field(name="\u200b", value="\u200b", inline=True)
